...
class processor implements \core_analytics\classifier, \core_analytics\regressor, \core_analytics\packable {
	...
	$rmse=array()
	 ...
	 // Evaluate the model multiple times to confirm the results are not significantly random due to a short amount of data.
        for ($i = 0; $i < $niterations; $i++) {
            ...
            $rmse[] = $this->get_rmse($data->getTestLabels(), $predictedlabels);	// a function to calculate the root mean square error
        }

        // Let's fill the results changing the returned status code depending on the phi-related calculated metrics.
        return $this->get_evaluation_result_object_regressor($dataset, $rmse, $maxdeviation);
     ...
     */
     ...
     protected function get_evaluation_result_object_regressor(\stored_file $dataset, $rmse, $maxdeviation) {

        // Average rmse of all evaluations
        $avgrmse = \Phpml\Math\Statistic\Mean::arithmetic($rmse);
      
	//  The standard error of the model is our estimate of the standard deviation of the noise in the model.
        $modeldev = $avgrmse;
    ...
    /* Returns the root mean square error in the prediction
     * @param array $a
     * @param array $b
     *
     * @return float
    ...
    protected function get_rmse($testlabels, $predictedlabels) {
	...
	for ($i = 0; $i < $count; ++$i) {
            $rmse += abs($testlabels[$i] - $predictedlabels[$i]) ** 2;
            }
	$rmse=$rmse/$count;
	$rmse=sqrt($rmse);
